...
- [Chrome: Stronger With Every Update](https://blog.google/security/chrome-stronger-with-every-update/) - How Chrome put agents across discovery, triage, fixing, and release, fixing 1,072 security bugs in two milestones and finding a sandbox escape that had survived 13 years.
...
- [River: Autonomous Vulnerability Remediation](https://shopify.engineering/river-vulnerability-remediation) - Shopify's Slack-native remediation agent that revalidates patches against repository head, PR state, and tracker before acting, cutting the dependency vulnerability backlog about 70% in its first 11 days.
- [The Defense Factory](https://openai.com/the-defense-factory/) - OpenAI's reference architecture for a continuous inventory, discovery, dynamic validation, ownership, and verified remediation loop, with sprint numbers: 37% of findings deduplicated, 0.81% false positives after runtime validation, 100% Codex-generated patches, 0.53% rolled back.
